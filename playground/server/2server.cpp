        ...
        switch (c->conn_state) {
			...
			c->batch_count++;
		            ...
		            //if(c->thread_index == 288)
                            //cout<<"begin"<<endl;
                            int ret = write(c->recv_sfd/*+port_num/2*/, c->ret_buf + c->ret_buf_offset, c->ret_bytes);
                            if (ret <= 0) {
                                if (errno == EWOULDBLOCK || errno == EAGAIN) {
            ...
            case conn_closing : {
                 ...
                 //cout<<"batch"<<endl;
                 while(true){
                    int ret = write(c->recv_sfd, c->ret_buf + c->ret_buf_offset, c->ret_bytes);               
                    if (ret == c->ret_bytes) {
                                    //finish write ,ready to parse new instruction
                                    c->batch_count = 0;
                                    c->ret_buf_offset = 0;
...
//    printf("[%d:%d] starting working,worked bytes:%d \n",c->thread_index, c->sfd, c->bytes_processed_in_this_connection);
    ...
    for(int i =0;i<c->num;i++){
        ...
        if((b + c->num/*server_num*/)== 0)
           std::this_thread::sleep_for( std::chrono::nanoseconds(5));
        if(t==0){
            for(int i =0;i<c->num/*server_num*/;i++){
                c->aq[i].clean();
            }
            //cout<<"end"<<endl;
            //break;
