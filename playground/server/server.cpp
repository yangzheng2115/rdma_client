...
static void con_ret_buf(CONNECTION *c) {
    ...
    resp_head.totalbodylen = htons(c->key.length() + c->value.length());
    resp_head.threadnum = htons(c->binary_header.threadnum);
    resp_head.time = htonl(c->binary_header.time);
...
static int try_read_head_binary(CONNECTION *c) {
    ...
    c->binary_header.totalbodylen = ntohs(req->totalbodylen);
    c->binary_header.threadnum = ntohs(req->threadnum);
    c->binary_header.time = ntohl(req->time);
...
void *worker(void *args) {
    ...
    t = id /96;
    if(id%96 < 48){
	    //pin_to_core(parms[48 *(3+t)].core);
	    pin_to_core(parms[id % 96 + 48 *(3+t)].core);
	    //cout<<id % 96 + 48 *(3+t)<<endl;
    }	    
    else{
	    if(t == 3){
		    //pin_to_core(parms[48 * 7].core);
		    pin_to_core(parms[id % 96+ 48 *6].core);
		    //cout<<id % 96+ 48 *6<<endl;
	    }
	    else{
		    //pin_to_core(parms[48 * t].core);
		    pin_to_core(parms[id % 96 - 48 + 48 * t].core);
		    //cout<<id % 96- 48 + 48 * t<<endl;
	    }
    }       
...
void reset_cmd_handler(CONNECTION *c) {
    ...
    if (c->remaining_bytes > 0) {
        ...
        conn_state_jump(c->conn_state, /*conn_waiting*/conn_read);
    }
}
...
enum try_read_result try_read_network(CONNECTION *c) {
    ...
    */
    CONNECTION *b = conns[c->thread_index];
    if (b == NULL) {
            if (!(b = new(std::nothrow) CONNECTION)) {
                fprintf(stderr, "Failed to allocate connection object\n");
                //return;
            }
    }
    conns[c->thread_index] = b;
    int i = c->thread_index;
    uint32_t avail = c->read_buf_size;
    /*while(true){
         int ret = read(c->sfd, c->read_buf + c->remaining_bytes, 4-c->remaining_bytes);
	 if(ret == 0){
            res = ret;
            goto e;	    
	 }
	 else if(ret > 0){
               c->remaining_bytes += ret;
	 }
	 if(c->remaining_bytes == 4){
	       c->remaining_bytes = 0;
	       uint32_t s = *(uint32_t *)c->read_buf;
	       avail = ntohl(s);
	       //cout<<avail<<endl;
	       break;
	 }
    ...
    //int avail = c->read_buf_size * 2;
    int f = 0;
    while(true){
       //int ret = read(c->sfd, c->read_buf + c->remaining_bytes, avail);
       int ret = read(c->sfd, con_buf[i] + con_off[i]+c->remaining_bytes, avail);
       if(ret == 0){
	       res =ret;
	       break;
       }
       ...
       /*if(c->remaining_bytes % 40 == 0 && c->remaining_bytes != 0){
              protocol_binary_request_header *rd;
              c->working_buf = con_buf[i] + con_off[i]+c->remaining_bytes -40;
	      rd = (protocol_binary_request_header *) c->working_buf;
	      uint16_t bodylen = ntohs(rd->totalbodylen);
	      if(bodylen == 0){
		      avail =0;
		      c->remaining_bytes -= 40;
              }		      
       }*/ 
       if(c->remaining_bytes >= 16 && f == 0){
	      f++;
              uint32_t rd = *(uint16_t *)(con_buf[i] + con_off[i]+6);
              uint16_t bodylen = ntohs(rd);
	      //cout<<bodylen<<endl;
	      //cout<<avail<<" "<<c->read_buf_size<<endl;
              avail = bodylen - c->read_buf_size + avail;
	      //cout<<avail<<endl;
       }
       if (avail == 0){
              //cout<<"read"<<c->remaining_bytes<<endl;
              res = c->remaining_bytes;
	      int t = con_num[i];
	      con_sfd[i][t] = c->sfd;
	      protocol_binary_request_header *req;
	      c->working_buf = con_buf[i] + con_off[i];
              req = (protocol_binary_request_header *) c->working_buf;
              Item item;
              item.time = ntohl(req->time);
	      item.sfd = c->sfd;
	      item.offset = con_off[i];
	      item.count = res;
              b->time_vec.push_back(item);
	      con_off[i] += c->remaining_bytes;
	      con_num[i]++;
	      break;
       }
    }
    if(res>0&&con_num[i]!=server_num){
            res = -1;
	    c->remaining_bytes = 0;
            c->worked_bytes = 0;
	    return gotdata;
    ...
    //c->working_buf = c->read_buf;
    c->working_buf = con_buf[i];
    if(res>0){
      //res = c->read_buf_size * con_num[i];
      res = con_off[i];
      sort(b->time_vec.begin(),b->time_vec.end());
      /*cout<<b->time_vec[0].time<<endl;
      cout<<b->time_vec[1].time<<endl;
      b->time_vec[0].time = 5;
      b->time_vec[1].time = 2;
      sort(b->time_vec.begin(),b->time_vec.end());
      cout<<b->time_vec[0].time<<endl;
...
void conn_close(CONNECTION *c) {
    ...
    //if(cl[c->thread_index/server_num].load()==num/*server_num*/){
       close(c->sfd);
       //cout<<"close"<<endl;
        ...
        switch (c->conn_state) {
            ...
            case conn_deal_with : {
                ...
                switch (c->work_state) {
		            ...
		            if(con_num[c->thread_index] == 0){
                                conn_state_jump(c->conn_state, /*conn_waiting*/conn_read);
                                break;
			    }else{
			        CONNECTION *b = conns[c->thread_index];
				c->remaining_bytes = b->time_vec.front().count;
				c->working_buf = con_buf[c->thread_index] + b->time_vec.front().offset;
			    }
                        }
			/*if(c->worked_bytes % c->read_buf_size == 0){
                            CONNECTION *b = conns[c->thread_index];
                            int i = c->worked_bytes / c->read_buf_size;
                            int t = b->time_vec[i].index;
                            c->working_buf = con_buf[c->thread_index]+t*c->read_buf_size;
                        }*/
                    ...
                    case read_key_value : {
                        ...
                        if (re == 0) {
                            //wait for more data
                            conn_state_jump(c->conn_state, conn_read/*conn_waiting*/);
                        } else if (re == -1) {
			...
			//       cout<<c->value.c_str()<<endl;
		        //w.fetch_add(1);	
                        ...
                        //cout<<c->bytes_processed_in_this_connection<<endl;
                        //work_state_jump(c->work_state, parse_head);
                        //conn_state_jump(c->conn_state, conn_new_cmd);
                        c->batch_count++;
                        //cout<<c->batch_count<<endl;
                        con_ret_buf(c);
                        break;
                    }
                    case query_key : {
			...
			c->bytes_processed_in_this_connection +=
                                sizeof(c->binary_header) + c->binary_header.totalbodylen;
			//if(w.load()==0)
	            	//		cout<<context.output_bytes<<endl;		
                        if (stat == Status::Ok) {
                            //  std::cout << ":find" << c->value << endl;
                            c->query_hit = false;//true;
			    //m.fetch_add(1);
                        } else {
			...
			if (c->remaining_bytes < 1 ) {
                        //if(c->binary_header.batchnum == c->batch_count) {
                                    /*c->batch_count = 0;
                                    c->ret_buf_offset = 0;
                                    c->ret_bytes = 0;
                                    cout<<c->bytes_processed_in_this_connection<<endl;
                                    ...
                                    conn_state_jump(c->conn_state, conn_new_cmd);
                             break;*/
		            //if(c->thread_index == 288)
                            //cout<<"begin"<<endl;
			    int sfd;
			    CONNECTION *b = conns[c->thread_index];
			    sfd = b->time_vec.front().sfd;
                            /*if(c->worked_bytes % c->read_buf_size == 0){
                                 CONNECTION *b = conns[c->thread_index];
                                 int i = c->worked_bytes / c->read_buf_size -1;
                                 sfd = b->time_vec[i].sfd;
                            }*/
			    /*if(c->worked_bytes>c->read_buf_size)
			        sfd = con_sfd[c->thread_index][1];
			    else
				sfd = con_sfd[c->thread_index][0];*/
                            int ret = write(sfd/*+port_num/2*/, c->ret_buf + c->ret_buf_offset, c->ret_bytes);
                            if (ret <= 0) {
                                if (errno == EWOULDBLOCK || errno == EAGAIN) {
                                    conn_state_jump(c->conn_state, conn_deal_with/*conn_waiting*/);
                                } else {
                                    cout<<c->thread_index<<"  "<<c->bytes_processed_in_this_connection<<" "<<c->bytes_wrote_back_in_this_connection<<endl;
                                    perror("write error");
                                    conn_state_jump(c->conn_state, conn_closing);
                                }
                            } else {
                                if (ret == c->ret_bytes) {
                                    ...
                                    c->ret_buf_offset = 0;
                                    c->ret_bytes = 0;
				    con_num[c->thread_index]--;
				    auto item = b->time_vec.begin();
                                    b->time_vec.erase(item);
                                    work_state_jump(c->work_state, parse_head);
                                    conn_state_jump(c->conn_state, conn_new_cmd);
				...
				if(con_num[c->thread_index] == 0){
				     b->time_vec.clear();
				     con_off[c->thread_index] = 0;
				}     
                                //cout<<c->bytes_wrote_back_in_this_connection<<endl;
                            }
                        }
                        else{
                            work_state_jump(c->work_state, parse_head);
...
void event_sendtime(const int fd, const short which, void *arg) {
    ...
    if(avail == 0){
        //timestamp.store(18930);
        *(uint32_t *) c->ret_buf = htonl(timestamp.load());
        write(c->sfd,c->ret_buf,4);
...
//    printf("[%d:%d] starting working,worked bytes:%d \n",c->thread_index, c->sfd, c->bytes_processed_in_this_connection);
    ...
    for(int i =0;i<1;i++){
        ...
        //if((b + c->num/*server_num*/)== 0)
        //   std::this_thread::sleep_for( std::chrono::nanoseconds(5));
        if(t==0){
            for(int i =0;i<1/*server_num*/;i++){
                c->aq[i].clean();
            }
            //cout<<"end"<<endl;
            //break;
...
void conn_new(int sfd, struct event_base *base, int thread_index) {
	...
	static struct sockaddr_in srv_addr1;
        //create  socket
        ...
        string server_ip;
	string sin;
	if(thread_index / 96 == 0){
            server_ip = "172.168.204.75";
	    sin = "172.168.204.90";
	}
	else if(thread_index / 96 == 1){ 
	    server_ip = "172.168.204.76";
	    sin = "172.168.204.97";
	}
	else if(thread_index / 96 == 2){
            server_ip = "172.168.204.77";
	    sin = "172.168.204.98";
	}
	else{
	    server_ip = "172.168.204.78";
	    sin = "172.168.204.99";
	}
        srv_addr.sin_family = AF_INET;
        srv_addr.sin_port = htons(PORT_BASE + thread_index % 96);
        srv_addr.sin_addr.s_addr = inet_addr(server_ip.c_str());
        cout<<thread_index<<endl;

        srv_addr1.sin_family = AF_INET;
        srv_addr1.sin_port = htons(PORT_BASE + thread_index);
        srv_addr1.sin_addr.s_addr = inet_addr(sin.c_str());

        if(bind(connect_fd,(struct sockaddr *) &srv_addr1 , sizeof(srv_addr1))){
        ...
        if( connect(connect_fd, (struct sockaddr*)&srv_addr, sizeof(srv_addr)) == -1) {
          ...
          if (fcntl(connect_fd, F_SETFL, fcntl(connect_fd, F_GETFL) | O_NONBLOCK) < 0) {
           ...
           cout<<"success "<<connect_fd<<"  "<<thread_index<<endl;
        */
        if (c == NULL) {
	...
	if(con_buf[thread_index]==NULL)
	{
	   cout<<"init con_buf"<<endl;
	   con_num[thread_index] = 0;
	   con_off[thread_index] = 0;
	   con_buf[thread_index] = (char *) malloc(c->read_buf_size*server_num*2);
	}
	//c->recv_sfd = r_sfd[thread_index];
        c->sfd = sfd;
        c->worked_bytes = 0;
        c->working_buf = c->read_buf;
        c->recv_bytes = 0;
	...
	if(thread_index < THREAD_NUM-1)
        event_set(&c->event, c->sfd, EV_READ | EV_PERSIST, event_handler, (void *) c);
            ...
            event_set(&c->event, c->sfd, EV_READ | EV_PERSIST, event_sendtime, (void *) c);
...
void conn_dis(evutil_socket_t listener, short event, void *args) {
...
void conn_dispatch(evutil_socket_t listener, short event, void *args) {
    ...
    string server_ip = "172.168.204.78";
    srv_addr.sin_family = AF_INET;
    srv_addr.sin_port = htons(PORT_BASE + port_index);
    srv_addr.sin_addr.s_addr = inet_addr(server_ip.c_str());
