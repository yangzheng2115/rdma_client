    ...
    //res = c->aq[0].pread(&c->read_buf, c->worked_bytes,flag);
        if(flag == 10){
            ...
            //c->active.notify();
                ...
                // m.fetch_add(1);
    while(true){
       //int ret = read(c->sfd, c->read_buf, c->read_buf_size);
       //int ret = read(c->sfd, con_buf[i], c->read_buf_size);
    ...
    while (!stop) {
        ...
        switch (c->conn_state) {
			...
			c->batch_count++;
			    ...
			    //int ret = write(c->sfd/*+port_num/2*/, c->ret_buf + c->ret_buf_offset, c->ret_bytes);
                            int ret = write(c->sfd/*+port_num/2*/, con_buf[c->thread_index] + c->ret_buf_offset, c->ret_bytes);
			    if (ret <= 0) {
                                if (errno == EWOULDBLOCK || errno == EAGAIN) {
