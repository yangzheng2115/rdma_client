...
enum try_read_result try_read_network(CONNECTION *c) {
    ...
    */
    ...
    //res = c->aq[0].pread(&c->read_buf, c->worked_bytes,flag);
        if(flag == 10){
            //c->full.store(false);
            //c->active.notify();
        }
    ...
    while(true){
       ...
       int ret = read(c->sfd, con_buf[i] + c->read_buf_size * con_num[i]+c->remaining_bytes, avail);
       if(ret == 0){
	       res =ret;
	       break;
       }
       ...
       if (avail == 0){
	      ...
	      con_sfd[i][t] = c->sfd;
	      protocol_binary_request_header *req;
	      c->working_buf = con_buf[i] + c->read_buf_size * con_num[i];
              req = (protocol_binary_request_header *) c->working_buf;
              Item item;
...
void conn_new(int sfd, struct event_base *base, int thread_index) {
	...
	if(con_buf[thread_index]==NULL)
	{
	   ...
	   con_buf[thread_index] = (char *) malloc(c->read_buf_size*server_num);
	}
