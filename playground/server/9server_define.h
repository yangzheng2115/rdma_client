...
class atomiccirculequeue {
    constexpr static size_t limit = 1024;//(batch_size * 2);
    ...
    inline int pread(char **buf, int worker , int &f) {
        ...
        */
        ...
        if ((b - a) == 1) {
            ...
            if(f == 1)
                count++;
            //cout<<h<<"  "<<read_offset<<" "<<a<<" "<<b<<" "<<worker<<endl;
        }
        if(count==5) {
            f = 10;
            count=0;
        ...
        //cout<<h<<"  "<<read_offset<<" "<<worker<<endl;
        //read_offset = read_offset % (limit * INIT_READ_BUF_SIZE);
