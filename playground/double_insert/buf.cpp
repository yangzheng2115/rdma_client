...
void data_send(int tid) {
    ...
    Tracer t;
    t.startTime();
    char rec_buf[SEND_BATCH * 2 * PACKAGE_LEN];
    int recv_offset = 0;
    int offset = 0;
    uint32_t s=0;
    uint64_t snum = 0;
    ...
    *(uint16_t *) HEAD_BODY_LENGTH(dummy) = htons(0);
    ...
    for(int i =0;i<myround;i++){
        while(num!=data_offset){
            ...
            //*(uint16_t *) HEAD_PRE_HASH(send_database[id]+h*WORK_OP_NUM) = htons(s);
            //continue;
            //int ret = write(cons[id].get_fd(), send_database[id] + h*WORK_OP_NUM, s);
            //int ret = write(cons[id].get_fd(), send_database[id], s);
            //cout<<"write"<<endl;
            if (ret != s) {
            ...
            while(true){
                 ret = read(cons[id].get_fd(), rec_buf + recv_offset, s - recv_offset);
                 if(ret>0){
                   recv_bytes[id] += ret;
                   //recv_offset = 0;
                   //break;
