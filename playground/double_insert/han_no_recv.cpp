    ...
    *(uint16_t *) HEAD_PRE_HASH(dummy) = htons(0);
    ...
    for(int i =0;i<myround;i++){
        while(num!=data_offset){
            ...
            *(uint16_t *) HEAD_PRE_HASH(send_database[id+96]+h*WORK_OP_NUM) = htons(s);
            ret = write(cons[id+96].get_fd(), send_database[id+96] + h*WORK_OP_NUM, s);
            s = thread_offset[id+192][h];
            *(uint16_t *) HEAD_PRE_HASH(send_database[id+192]+h*WORK_OP_NUM) = htons(s);
            ret = write(cons[id+192].get_fd(), send_database[id+192] + h*WORK_OP_NUM, s); 
            s = thread_offset[id+288][h];
            *(uint16_t *) HEAD_PRE_HASH(send_database[id+288]+h*WORK_OP_NUM) = htons(s);
