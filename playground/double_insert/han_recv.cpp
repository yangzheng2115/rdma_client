...
void data_send(int tid) {
    ...
    while (send_stop.load() != thread_num) {
        for (int i = 0; i < thread_num; i++) {
            if (local_cons[i][id].flag.load() == 1) {
                //cout<<id<<endl;
                while(true){
                int ret = write(cons[id].get_fd(), local_cons[i][id].get_buf()+offset, 10*PACKAGE_LEN/*local_cons[i][id].get_offset()-offset*/);
                if(ret >0){
                   offset+=ret;
                   while(true){
...
void data_recv(int tid) {
    ...
    while (true) {
        ...
        int ret = read(recv_sfd[id], rec_buf + offset, 10 * PACKAGE_LEN - offset);
        if (ret == 0) {
            //if(id == 2)
        ...
        if (ret == -1) {
           ...
           //}
            ...
            offset += ret;
            if (offset == 10 * PACKAGE_LEN) {
                
                recv_flag[id].fetch_add(1);
                //if(id<6)
                //cout<<recv_flag[id].load()<<endl;
                offset = 0;
