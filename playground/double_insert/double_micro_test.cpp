...
void con_database() {
    ...
    //uint64_t num = 0;
    for (size_t i = number*count/client_num; i < (number+1)*count/client_num/*KV_NUM*/; i++) {
        ...
        *(uint32_t *) HEAD_TIME(package_buf) = htonl(Time);
        ...
        memcpy(value_buf, (uint8_t *) loads[i]->getVal(), 8/*VALUE_LEN*/);
...
void data_push(int tid) {
   ...
   int kv_num = KV_NUM /client_num;
    while (!stop) {
        if (WORK_OP_NUM > /*KV_NUM*/kv_num - g_count) {
            printf("myround %d end,g_count %d\n", round, g_count);
            if (--round <= 0) {
                stop = true;
            } else {
        ...
        for (int i = 0; i < WORK_OP_NUM; i++) {
            ...
            *(uint16_t *) HEAD_BODY_LENGTH(send_database[pre_hash] + t*WORK_OP_NUM+send_offset[pre_hash]) = htons(total_body);
            //memcpy(send_database[pre_hash] + send_offset[pre_hash], p.package_ptr, p.package_len);
            send_offset[pre_hash] += p.package_len;
            thread_recv[pre_hash][t]+=p.package_len-/*myround*/20 + round;
            //g_offset += p.package_len;
        }
        for (int i = 0; i < port_num; i++) {
        ...
        //if(g_count == 250000)
        //    continue;
        if(snum == (250000 / client_num)/ SEND_BATCH * myround)
            break;
    }
}
...
void data_send(int tid) {
    ...
    *(uint16_t *) HEAD_PRE_HASH(dummy) = htons(0);
    ...
    //cout<<"begin"<<endl;
    recv_bytes[id] = 0;
    uint64_t data_offset = 250000 / client_num;//send_offset[id];
    uint64_t num = 0;
    //goto y;
    for(int i =0;i<myround;i++){
        ...
        num = 0;
        data_offset =  250000 / client_num;//send_offset[id];
        offset = 0;
        if(id == 0)
            cout<<i<<endl;
    }
