...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < port_num; i++) {
        send_database[i] = (char *) calloc(KV_NUM, PACKAGE_LEN);
...
void con_database() {
    ...
    for (size_t i = 0; i < count/*KV_NUM*/; i++) {
        ...
        Total_body_length = strlen(loads[i]->getKey());
...
void con_database_run() {
    ...
    uint64_t range = KEY_RANGE;
    uint64_t count = KV_NUM;
    ycsb::YCSBLoader runner(ycsb::runpath, count);
    loads = runner.load();
    count = runner.size();
    cout << "count:" << count << endl;
    my_database = (char *) calloc(KV_NUM, PACKAGE_LEN);
    ...
    for (size_t i = 0; i < count/*KV_NUM*/; i++) {
        ...
        //uint64_t n = array[i] / 26;
        //uint8_t c = array[i] % 26;
        if(static_cast<int>(loads[i]->getOp())!= 0){
            //if(i <100)
            //   cout<<static_cast<int>(loads[i]->getOp())<<endl;
            continue;
        }
        memset(package_buf, 0, sizeof(package_buf));
        memset(key_buf, 0, sizeof(key_buf));
...
void data_push(int tid) {
    ...
    while (!stop) {
        ...
        for (int i = 0; i < WORK_OP_NUM; i++) {
            ...
            memcpy(send_database[pre_hash] + t*WORK_OP_NUM+send_offset[pre_hash], p.package_ptr, p.package_len);
            //memcpy(send_database[pre_hash] + send_offset[pre_hash], p.package_ptr, p.package_len);
            send_offset[pre_hash] += p.package_len;
            g_offset += p.package_len;
            thread_recv[pre_hash][t]++;
        }
        for (int i = 0; i < port_num; i++) {
        ...
        //if(g_count == 250000)
        //    continue;
        if(snum == 250000 / SEND_BATCH * myround)
            break;
    }
}
...
void data_gettime(int tid) {
    ...
    uint64_t h, t;
     while(stop_time.load() == 0){
           ...
           //     break;
          int j =0;
           while(send_stop.load()%(/*port_num*/1+1)!=0)
           {  
                  j++;
                  if(j == 100000)
...
void data_send(int tid) {
    ...
    Tracer t;
    t.startTime();
    char rec_buf[SEND_BATCH * 2 * (PACKAGE_LEN + VALUE_LEN)];
    int recv_offset = 0;
    int offset = 0;
    uint32_t s=0;
    uint64_t snum = 0;
    ...
    *(uint16_t *) HEAD_BODY_LENGTH(dummy) = htons(0);
    char * size_buf = (char *) calloc(1, PACKAGE_LEN);
    if(id>=1)
        return ;
    //cout<<"begin"<<endl;
    recv_bytes[id] = 0;
    uint64_t data_offset = 250000;//send_offset[id];
    uint64_t num = 0;
    /*write(cons[id].get_fd(), send_database[id], SEND_BATCH * 1 * PACKAGE_LEN);
    while(true){
         int re = read(cons[id].get_fd(), rec_buf + recv_offset, SEND_BATCH * 1 * PACKAGE_LEN - recv_offset);
         if(re>0){
                 //recv_bytes[id] += re;
                 recv_offset += re;
                 send_bytes[id] += re;
         }
         if (recv_offset == SEND_BATCH * 1 * PACKAGE_LEN) {
    ...
    for(int i =0;i<myround;i++){
        while(num!=data_offset){
            ...
            send_stop.fetch_add(1);
            snum++;
            //if(id == 0 && snum == 250000 / SEND_BATCH * myround)
            //        cout<<"0 end"<<recv_bytes[id]<<endl;
            ...
            // if(snum % 1000 == 0 && id == 0) 
            //      cout<<snum<<endl;
            /*send_start.fetch_add(1);
            uint64_t sa = send_start.load();
            while(sa%port_num!=0)
            {
            ...
            //s = 40;
            //s = send_offset[id];
            //*(uint16_t *) HEAD_PRE_HASH(send_database[id]) = htons(s);
            //s = *(uint64_t *)(change_offset[id]+8*num) - offset;
            //*(uint16_t *) HEAD_PRE_HASH(send_database[id]+offset) = htons(s);
            //s = *(uint64_t *)(change_offset[id]+8*num) - offset;
            //*(uint32_t *)size_buf  = htonl(s);
            //cout<<s<<endl;
            //write(cons[id].get_fd(), size_buf, 4);
            ...
            /*if(snum % 2 == 1){
               *(uint16_t *) HEAD_PRE_HASH(send_database[id]) = htons(s);
               ret = write(cons[id].get_fd(), send_database[id], s);
            }else{
               *(uint16_t *) HEAD_PRE_HASH(push_database[id]) = htons(s);
                ret = write(cons[id].get_fd(), push_database[id], s);
            }*/
            if(s == 0){
               write(cons[id].get_fd(), dummy, HEAD_LEN);
               continue;
            }
            *(uint32_t *) HEAD_PRE_HASH(send_database[id]+h*WORK_OP_NUM) = htonl(s);
            //continue;
            int ret = write(cons[id].get_fd(), send_database[id] + h*WORK_OP_NUM, s);
            //cout<<"send "<<ret<<"  "<<s<<endl;
            /*s = thread_offset[id+96][h];
            *(uint32_t *) HEAD_PRE_HASH(send_database[id+96]+h*WORK_OP_NUM) = htonl(s);
            ret = write(cons[id+96].get_fd(), send_database[id+96] + h*WORK_OP_NUM, s);
            s = thread_offset[id+192][h];
            *(uint32_t *) HEAD_PRE_HASH(send_database[id+192]+h*WORK_OP_NUM) = htonl(s);
            ret = write(cons[id+192].get_fd(), send_database[id+192] + h*WORK_OP_NUM, s);
            s = thread_offset[id+288][h];
            *(uint32_t *) HEAD_PRE_HASH(send_database[id+288]+h*WORK_OP_NUM) = htonl(s);
            ret = write(cons[id+288].get_fd(), send_database[id+288] + h*WORK_OP_NUM, s);
            */if (ret != s) {
            ...
            g_totalbytes += ret;
            //if(id == 0 && snum == 250000 / SEND_BATCH * myround)
            //cout<<"0 end"<<s + VALUE_LEN * thread_recv[id][h]<<endl;
            //send_offset[id] = 0;
            //send_bytes[id] += ret;
            //offset += ret;
            //write(cons[id].get_fd(), dummy, PACKAGE_LEN);
            //local_cons[i][id].flag.fetch_sub(1);
            //cout<<"recv "<<s + VALUE_LEN * thread_recv[id][h]<<endl;
            for(int j =0;j<4;j+=96){
            while(true){
                 s= thread_offset[id+j][h] - 16;
                 ret = read(cons[id+j].get_fd(), rec_buf + recv_offset, s + VALUE_LEN * thread_recv[id+j][h] - recv_offset);
                 if(ret>0){
                   //if(id == 0 && snum == 250000 / SEND_BATCH * myround)
                   //         cout<<"0 end"<<ret<<endl;
                   recv_bytes[id] += ret;
                 ...
                 if (recv_offset == s + VALUE_LEN * thread_recv[id+j][h]) {
                      //recv_flag[id].fetch_add(1);
                      //cout<<"recv"<<endl;
                      //if(id == 0 && snum == 250000 / SEND_BATCH * myround)
                      //      cout<<"0 end"<<recv_bytes[id]<<endl;
            ...
            //send_stop.fetch_add(1);
            //snum++;
            /*if(g_totalbytes == 10000000*myround){
                    break;
                    recv_flag[1].fetch_add(1);
                    if(id == 1)
                        cout<<"1 "<<send_stop.load()%port_num<<endl;
            }*/
            /*if(id == 0){
               while(send_stop.load()%port_num!=0)
               {
                  //if(recv_flag[1].load() == 384){
                  //        break;
                  //}
                  std::this_thread::sleep_for( std::chrono::nanoseconds(1));
               }
               //if(snum < 500){
               //    cout<<send_stop.load()%port_num<<endl;
               //    cout<<send_stop.load()<<endl;
               //}
            ...
            /*if(g_totalbytes == 10000000*myround){
                    break;
                    recv_flag[0].fetch_add(1);
                    if(recv_flag[0].load() == 384){
                          cout<<"end"<<endl;
                          cout<<send_stop.load()%port_num<<endl;
                          cout<<send_start.load()%port_num<<endl;
                          cout<<send_stop.load()<<endl;
                          cout<<send_start.load()<<endl;
                    }
            }
            uint64_t  st = send_stop.load();
            while(st%port_num!=0)
            {
                st = send_stop.load();
                //if(id == 0)
            ...
            //    cout<<"stop: "<<st<<endl;
            /*send_start.fetch_add(1);
            while(send_start.load()%port_num!=0)
            {
                //std::this_thread::yield();
                std::this_thread::sleep_for( std::chrono::nanoseconds(1));
            }*/
            /*send_start.fetch_add(1);
            j = 0;
            while(send_start.load()!=96){
                 j++;
                if(j == 1000000)
                    cout<<"start"<<send_start.load()<<endl;
                 std::this_thread::sleep_for( std::chrono::nanoseconds(1));
            }
            if(id == 0){
              send_stop.store(0);  
        ...
        num = 0;
        data_offset =  250000;//send_offset[id];
        offset = 0;
        if(id == 0)
            cout<<i<<endl;
    }
