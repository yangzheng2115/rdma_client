...
void data_gettime(int tid) {
    ...
    uint64_t h, t;
     while(stop_time.load() == 0){
           ...
           //     break;
          int j =0;
           while(send_stop.load()%(port_num+1)!=0)
           {  
                  j++;
                  if(j == 100000)
    ...
    for(int i =0;i<myround;i++){
        while(num!=data_offset){
            ...
            *(uint16_t *) HEAD_PRE_HASH(send_database[id]+h*WORK_OP_NUM) = htons(s);
            ...
            while(true){
                 ret = read(cons[id].get_fd(), rec_buf + recv_offset, s + VALUE_LEN * recv_num[id][h] - recv_offset);
                 if(ret>0){
                   recv_bytes[id] += ret;
                   //recv_offset = 0;
                   //break;
                 ...
                 if (recv_offset == s + VALUE_LEN * recv_num[id][h]) {
                      //recv_flag[id].fetch_add(1);
                      //cout<<"recv"<<endl;
                      recv_num[id][h] = 0;
                      recv_offset = 0;
