...
void con_database() {
    ...
    uint8_t Pre_hash = 0;
    uint8_t Retain = 0;
    uint32_t Total_body_length = KEY_LEN;
    ...
    while (send_stop.load() != port_num) {
        for (int i = 0; i < port_num; i++) {
            if (local_cons[i][id].flag.load() == 1) {
                while(true){
                int ret = write(cons[id].get_fd(), local_cons[i][id].get_buf()+offset, 500*PACKAGE_LEN/*local_cons[i][id].get_offset()-offset*/);
                if(ret >0)
                   offset+=ret;
...
void data_recv(int tid) {
    ...
    char rec_buf[500 * 36/*PACKAGE_LEN*/];
    int offset = 0;
    int qw = 0;
    recv_bytes[id] = 0;
    while (true) {
        //int ret = read(cons[id/*+thread_num*/].get_fd()+2, rec_buf + offset, SEND_BATCH * PACKAGE_LEN - offset);
        int ret = read(recv_sfd[id], rec_buf + offset, 500 * 36/*PACKAGE_LEN*/ - offset);
        if (ret == 0) {
            //if(id == 2)
