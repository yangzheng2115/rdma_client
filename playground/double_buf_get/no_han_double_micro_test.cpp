...
const string server_ip = "172.168.204.97";//"127.0.0.1";
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < port_num; i++) {
        ...
        cons[i].init_server(i * 96 / port_num, server_ip);
        if (cons[i].get_fd() == -1) {
    ...
    for (int i = thread_num * 1; i < thread_num * 1 + 96; i++) {
        //printf("creating thread %d\n",i);
