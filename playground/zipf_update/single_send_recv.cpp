...
int main(int argc, char **argv) {
    ...
    recv_bytes = (uint64_t *) calloc(96, sizeof(uint64_t));
...
void con_database() {
    ...
    for (size_t i = 0; i < count/*KV_NUM*/; i++) {
        ...
        *(uint16_t *) HEAD_BATCH_NUM(package_buf) = htons(1/*SEND_BATCH*/);
...
void data_send(int tid) {
    ...
    int s=0;
    recv_bytes[id] = 0;
    uint64_t data_offset = send_offset[id];
    write(cons[id].get_fd(), send_database[id], SEND_BATCH * 2 * PACKAGE_LEN);
    while(true){
         int re = read(recv_sfd[id], rec_buf + recv_offset, SEND_BATCH * 2 * PACKAGE_LEN - recv_offset);
         if(re>0){                
                 //recv_bytes[id] += re;
                 recv_offset += re;
         }
         if (recv_offset == SEND_BATCH * 2 * PACKAGE_LEN) {
    ...
    for(int i =0;i<myround;i++){
        while(data_offset!=0){
            while(true){
                if(recv_flag[id].load()==1)
                  {
                      recv_flag[id].store(0);
                      //recv_flag[(id+1)%96].fetch_add(1);
                      break;
                  }
            ...
            while(true){
                 ret = read(recv_sfd[id], rec_buf + recv_offset, SEND_BATCH * PACKAGE_LEN - recv_offset);
                 if(ret>0){
                 recv_bytes[id] += ret;
                 recv_offset += ret;
                 }
