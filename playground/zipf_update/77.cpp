...
struct hashfunc
{
    ...
    {
        return MurmurHash64A(v.buf+HEAD_LEN,v.key_len);
    }
};
struct eqfunc
{
    bool operator()(const Item v1, Item v2) const
    {
        return (std::memcmp(v1.buf+HEAD_LEN,v2.buf+HEAD_LEN,v1.key_len) == 0);
    }
};

vector<vector <Item> > thread_item(400);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < port_num; i++) {
        send_database[i] = (char *) calloc(WORK_OP_NUM*limit, PACKAGE_LEN);
        //push_database[i] = (char *) calloc(KV_NUM, PACKAGE_LEN);
        //change_offset[i] = (char *) calloc(KV_NUM, 8);
        //thread_item[i].resize(WORK_OP_NUM*limit);
        //cout<<i<<endl;
...
void con_database() {
    ...
    for (size_t i = 0; i < count/*KV_NUM*/; i++) {
        ...
        if(static_cast<int>(loads[i]->getOp())!= 3){
            continue;
        }
        ...
        *(uint32_t *) HEAD_TIME(package_buf) = htonl(Time);
        ...
        uint8_t t =((hash_func(key_buf, KEY_LEN))%67108864)%(24);
        Pre_hash = static_cast<uint8_t > (((hash_func(key_buf, Key_length))%67108864) /(67108864/4));
        //Pre_hash = Pre_hash * (port_num / 4)+t;
        //Pre_hash = i % port_num;
        //if(Pre_hash < 2){
        //Pre_hash -=6;
        Pre_hash = Pre_hash * (24)+t;
...
void data_push(int tid) {
    uint64_t  snum  =0;
    int round = ROUND_SET;
    boost::unordered_map<string,int> map[400];
   for (int i = 0; i < port_num; i++) {
            send_offset[i] = 16;
   }
    while (!stop) {
        if (WORK_OP_NUM > /*45000000*/num - g_count) {
            printf("myround %d end,g_count %d\n", myround, g_count);
            if (--round <= 0) {
                stop = true;
            } else {
        ...
        for (int i = 0; i < WORK_OP_NUM; i++) {
            ...
            //Item item;
            //item.buf = my_database + g_offset;
            //item.pag_len = ntohs(body) + HEAD_LEN;
            ...
            //item.key_len = ntohs(key_len);
            //thread_item[pre_hash][send_offset[pre_hash]+t*WORK_OP_NUM] = item;
            string a =std::string(my_database + g_offset+HEAD_LEN,ntohs(key_len));
            if(map[pre_hash].find(a)!=map[pre_hash].end()){
                //if(map[a] == 0)
                //   cout<<a<<"  "<<map[a]<<endl;
            ...
            //memcpy(send_database[pre_hash] + send_offset[pre_hash], p.package_ptr, p.package_len);
            //send_offset[pre_hash]++;
            //g_offset += item.pag_len;
...
void data_send(int tid) {
    ...
    Tracer t;
    t.startTime();
    //char rec_buf[/*WORK_OP_NUM*/10000 * PACKAGE_LEN];
    char *rec_buf = (char *) calloc(WORK_OP_NUM, PACKAGE_LEN);
    int recv_offset = 0;
    int offset = 0;
    uint32_t s=0;
    uint64_t snum = 0;
    ...
    char *package_buf[4];
    for(int i = 0;i < 4;i++) 
         package_buf[i] = (char *) calloc(WORK_OP_NUM, PACKAGE_LEN);
    ...
    *(uint16_t *) HEAD_PRE_HASH(dummy) = htons(0);
    ...
    uint64_t num = 0;
    boost::unordered_map<Item,int,hashfunc,eqfunc> map[4];
    ...
    for(int i =0;i<myround;i++){
        while(num!=data_offset){
            ...
            /*for(int w = 0; w < 4;w++){
                sr[w] = 16;
                for(int j = 0; j<thread_offset[id+w*96][h];j++){
                  Item item = thread_item[id+w*96][j+h*WORK_OP_NUM];
                  //string a =std::string(item.buf+HEAD_LEN,item.key_len);
                  if(map[w].find(item)!=map[w].end()){
                     continue;
                  }
                  map[w][item] = j;
                  
                  memcpy(package_buf[w]+sr[w],item.buf,item.pag_len);
                  sr[w]+=item.pag_len;
               }
               //cout<<s<<endl;
               map[w].clear();
            ...
            int ret;
            //for(int w = 0;w<4;w++){
            //   *(uint32_t *) HEAD_PRE_HASH(package_buf[w]) = htonl(sr[w]);
            //   ret = write(cons[id+w*96].get_fd(), package_buf[w], sr[w]);
           // }
            s = thread_offset[id][h];
            if(h % 20 == 19){
               if(h != 19){
                  s = s - thread_offset[id][h-20];
                  sh = thread_offset[id][h-20];
               }
               else{
                 sh = 0;
               }  
            ...
            //*(uint32_t *) HEAD_PRE_HASH(package_buf) = htonl(s);
            *(uint32_t *) HEAD_PRE_HASH(send_database[id]+sh/*h*WORK_OP_NUM*PACKAGE_LEN*/) = htonl(s);
            //continue;
            ret = write(cons[id].get_fd(), send_database[id] + sh/*h*WORK_OP_NUM*PACKAGE_LEN*/, s);
            //cout<<"send"<<s<<endl;
