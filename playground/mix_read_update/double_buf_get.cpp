...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < port_num; i++) {
        ...
        thread_item[i].resize(WORK_OP_NUM*limit/4);
        cout<<i<<endl;
...
void con_database() {
    ...
    //loads = loader.load();
    //count = loader.size();
   char const *ba = "./run.dat";
    string tt = std::string(ba) + run_num;
    char const *da = tt.c_str();
    char path[100];
    strcpy(path,da);
    cout<<path<<endl;
    ycsb::YCSBLoader runner(/*ycsb::runpath*/path, count);
    loads = runner.load();
    count = runner.size();
   ...
   for (int i = 0; i < port_num; i++) {
    ...
    while (!stop) {
        ...
        for (int i = 0; i < WORK_OP_NUM; i++) {
            ...
            thread_item[pre_hash][send_offset[pre_hash]+t*(WORK_OP_NUM/4)] = item;
        ...
        //if(g_count == 250000)
        //    continue;
        if(snum == 125000 / SEND_BATCH * myround)
            break;
    }
}
...
void data_send(int tid) {
    ...
    *(uint16_t *) HEAD_PRE_HASH(dummy) = htons(0);
    ...
    //cout<<"begin"<<endl;
    recv_bytes[id] = 0;
    uint64_t data_offset = 125000;//send_offset[id];
    uint64_t num = 0;
    ...
    for(int i =0;i<myround;i++){
        while(num!=data_offset){
            ...
            for(int w = 0; w < 4;w++){
                ...
                for(int j = 0; j<thread_offset[id+w*96][h];j++){
                  Item item = thread_item[id+w*96][j+h*(WORK_OP_NUM/4)];
            ...
            //continue;
            //int ret = write(cons[id].get_fd(), package_buf/*send_database[id] + h*WORK_OP_NUM*PACKAGE_LEN*/, s);
            //cout<<"send"<<s<<endl;
            ...
            */
            ...
            if(ret > 0 ){
              ...
              s = kl[j/96]+VALUE_LEN*recv_num[j/96];
              if(s == 0)
                  s= 16;
              while(true){
        ...
        num = 0;
        data_offset =  125000;//send_offset[id];
        offset = 0;
        if(id == 0)
            cout<<i<<endl;
    }
