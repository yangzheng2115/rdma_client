...
void con_database() {
    ...
    for (size_t i = 0; i < count/*KV_NUM*/; i++) {
        ...
        *(uint16_t *) HEAD_PRE_HASH(package_buf) = Pre_hash;
        ...
        memcpy(send_database[Pre_hash] + send_offset[Pre_hash], package_buf, PACKAGE_LEN);
        send_offset[Pre_hash] += PACKAGE_LEN;
...
void data_send(int tid) {
    ...
    *(uint16_t *) HEAD_BODY_LENGTH(dummy) = htons(0);
    ...
    //cout<<"begin"<<endl;
    recv_bytes[id] = 0;
    uint64_t data_offset = PACKAGE_LEN * 250000;//send_offset[id];
    ...
    for(int i =0;i<myround;i++){
        while(data_offset!=0){
            s = SEND_BATCH*PACKAGE_LEN;
            if(data_offset < SEND_BATCH*PACKAGE_LEN)
                 break;
                 //s = data_offset;
            //if(g_totalbytes != 0){
            ...
            *(uint32_t *) HEAD_TIME(package_buf) = htonl(timestamp);
            ...
            while(true){
                 ret = read(cons[id].get_fd(), rec_buf + recv_offset, SEND_BATCH * PACKAGE_LEN - recv_offset);
                 if(ret>0){
                   recv_bytes[id] += ret;
                   //recv_offset = 0;
                   //break;
                 ...
                 if (recv_offset == SEND_BATCH * PACKAGE_LEN) {
                      //recv_flag[id].fetch_add(1);
                      //cout<<"recv"<<endl;
                      recv_offset = 0;
                      break;
        ...
        data_offset = PACKAGE_LEN * 250000;//send_offset[id];
        offset = 0;
        if(id == 0)
            cout<<i<<endl;
    }
