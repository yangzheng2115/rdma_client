...
TEST(CLASS, UpsertRead_Serial) {
    ...
    public:
        ...
        ReadContext(Key key, uint8_t expected)
                : key_{key}, expected_{expected} {
        }

        /// Copy (and deep-copy) constructor.
        ReadContext(const ReadContext &other)
                : key_{other.key_}, expected_{other.expected_} {
        }
...
TEST(CLASS, Rmw) {
    ...
    public:
        ...
        RmwContext(Key key, uint64_t incr)
                : key_{key}, incr_{incr}, val_{0} {
        }

        /// Copy (and deep-copy) constructor.
        RmwContext(const RmwContext &other)
                : key_{other.key_}, incr_{other.incr_}, val_{other.val_} {
        }
