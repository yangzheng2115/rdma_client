...
protected:
    File()
            : file_handle_{INVALID_HANDLE_VALUE}, device_alignment_{0}, filename_{}, owner_{false}
#ifdef IO_STATISTICS
    , bytes_written_ { 0 }
    , read_count_{ 0 }
    , bytes_read_{ 0 }
    ...
    File(const std::string &filename)
            : file_handle_{INVALID_HANDLE_VALUE}, device_alignment_{0}, filename_{filename}, owner_{false}
#ifdef IO_STATISTICS
    , bytes_written_ { 0 }
    , read_count_{ 0 }
    , bytes_read_{ 0 }
    ...
    /// Move constructor.
    File(File &&other)
            : file_handle_{other.file_handle_}, device_alignment_{other.device_alignment_},
              filename_{std::move(other.filename_)}, owner_{other.owner_}
#ifdef IO_STATISTICS
...
public:
    ...
    WindowsPtpThreadPool(WindowsPtpThreadPool &&other)
            : pool_{other.pool_}, callback_environment_{other.callback_environment_},
              cleanup_group_{other.cleanup_group_}, max_threads_{other.max_threads_} {
        other.pool_ = nullptr;
...
private:
    ...
    TaskStartSpringboard(PTP_CALLBACK_INSTANCE
    instance,
    PVOID parameter,
            PTP_WORK
    work);
...
public:
    ...
    static void CALLBACK
    IoCompletionCallback(PTP_CALLBACK_INSTANCE
    instance,
    PVOID context,
            PVOID
    overlapped,
...
public:
    ThreadPoolFile()
            : File(), io_object_{nullptr} {
    }

    ThreadPoolFile(const std::string &filename)
            : File(filename), io_object_{nullptr} {
    }

    /// Move constructor
    ThreadPoolFile(ThreadPoolFile &&other)
            : File(std::move(other)), io_object_{other.io_object_} {
    }
