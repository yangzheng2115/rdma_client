...
struct FlushCloseStatus {
    FlushCloseStatus()
            : flush{FlushStatus::Flushed}, close{CloseStatus::Closed} {
    }

    FlushCloseStatus(FlushStatus flush_, CloseStatus close_)
            : flush{flush_}, close{close_} {
    }
...
struct FullPageStatus {
    FullPageStatus()
            : LastFlushedUntilAddress{0}, status{} {
    }
...
public:
    ...
    PageOffset(const Address &address)
            : offset_{address.offset()}, page_{address.page()} {
    }
...
public:
    ...
    public:
        OnPagesClosed_Context(alloc_t *allocator_,
                ...
                : allocator{allocator_}, new_safe_head_address{new_safe_head_address_},
                  replace_with_clean_page{replace_with_clean_page_} {
        }

        /// The deep-copy constructor.
        OnPagesClosed_Context(const OnPagesClosed_Context &other)
                : allocator{other.allocator}, new_safe_head_address{other.new_safe_head_address},
    ...
    public:
        OnPagesMarkedReadOnly_Context(alloc_t *allocator_,
                ...
                : allocator{allocator_}, new_safe_read_only_address{new_safe_read_only_address_},
        ...
        OnPagesMarkedReadOnly_Context(const OnPagesMarkedReadOnly_Context &other)
                : allocator{other.allocator}, new_safe_read_only_address{other.new_safe_read_only_address},
...
private:
    inline void GetFileReadBoundaries(Address read_offset, uint32_t read_length,
        ...
        read_offset = Address{read_offset.page(),read_offset.offset()};
    ...
    if (context->allocator->MonotonicUpdate(context->allocator->safe_head_address,
        ...
        for (uint32_t idx = old_safe_head_address.page(); idx < context->new_safe_head_address.page();
            ...
            if (old_status.flush == FlushStatus::Flushed) {
                ...
                //aligned_free(context->allocator->Page(idx));
            }
        }
    }
}
...
Status PersistentMemoryMalloc<D>::AsyncFlushPages(uint32_t start_page, Address until_address,
    ...
    public:
        Context(alloc_t *allocator_, uint32_t page_, Address until_address_)
                : allocator{allocator_}, page{page_}, until_address{until_address_} {
        }

        /// The deep-copy constructor
        Context(const Context &other)
                : allocator{other.allocator}, page{other.page}, until_address{other.until_address} {
        }
...
Status PersistentMemoryMalloc<D>::AsyncReadPagesFromSnapshot(file_t &snapshot_file,
                                                             uint32_t file_start_page, uint32_t start_page,
                                                             uint32_t num_pages,
                                                             RecoveryStatus &recovery_status) {
...
inline void PersistentMemoryMalloc<D>::PageAlignedShiftHeadAddress(uint32_t tail_page) {
    ...
    desired_head_address += Address{0, 0, k}.control();
...
inline void PersistentMemoryMalloc<D>::ShiftHeadAddress(Address address) {
    ...
    if (MonotonicUpdate(head_address, address, old_head_address)) {
        ...
        //assert(result == Status::Ok);
        //epoch_->BumpCurrentEpoch(OnPagesClosed, context_copy);
        if (MonotonicUpdate(safe_head_address, address,
                            old_safe_head_address)) {
            for (uint32_t idx = old_safe_head_address.page(); idx < address.page();
                 ++idx) {
                FlushCloseStatus old_status = PageStatus(idx).status.load();
                FlushCloseStatus new_status;
                do {
                    new_status = FlushCloseStatus{old_status.flush, CloseStatus::Closed};
                } while (!PageStatus(idx).status.compare_exchange_weak(old_status,
                ...
                if (old_status.flush == FlushStatus::Flushed) {
                    // We closed the page after it was flushed, so we are responsible for clearing and
                    // reopening it.
                    std::memset(Page(idx), 0, kPageSize);
                    PageStatus(idx).status.store(FlushStatus::Flushed, CloseStatus::Open);
                    if (pages_[idx % buffer_size_]) {
                        aligned_free(pages_[idx % buffer_size_]);
                        pages_[idx % buffer_size_] = nullptr;
                    }
                }
            }
        }
...
inline void PersistentMemoryMalloc<D>::PageAlignedShiftReadOnlyAddress(uint32_t tail_page) {
    ...
    uint16_t k = current_read_only_address.h();
    ...
    desired_read_only_address += Address{0, 0, k}.control();
    Address old_read_only_address;
