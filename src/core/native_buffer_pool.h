...
public:
    /// Default constructor.
    SectorAlignedMemory()
            : buffer_{nullptr}, valid_offset{0}, required_bytes{0}, available_bytes{0}, level_{0}, pool_{nullptr} {
    }

    SectorAlignedMemory(uint8_t *buffer, uint32_t level, NativeSectorAlignedBufferPool *pool)
            : buffer_{buffer}, valid_offset{0}, required_bytes{0}, available_bytes{0}, level_{level}, pool_{pool} {
    }
    ...
    SectorAlignedMemory(SectorAlignedMemory &&other)
            : buffer_{other.buffer_}, valid_offset{other.valid_offset}, required_bytes{other.required_bytes},
              available_bytes{other.available_bytes}, level_{other.level_}, pool_{other.pool_} {
